...
/**
 * @author 26191568+jgwoolley@users.noreply.github.com This results object
 *         represents a single error for a specific file.
 */
public class FlowFileErrorResult {
	...
	final List<String> exceptionStack;
	...
	public FlowFileErrorResult(final Exception exception, @JsonProperty("path") final SourceFile path) {
		this(Arrays.asList(exception.getStackTrace()).stream().map(x -> x.toString()).collect(Collectors.toList()),
				exception.getMessage(), path);

	}

	@JsonCreator
	public FlowFileErrorResult(@JsonProperty("exceptionStack") final List<String> exceptionStack,
			@JsonProperty("exception") final String exception, @JsonProperty("path") final SourceFile path) {
